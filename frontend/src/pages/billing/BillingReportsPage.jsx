...
const BillingReportsPage = () => {
  ...
  const reportBuilderSteps = useMemo(() => [
    {
      title: 'Configure Report',
      description: 'Select type and fields',
    },
    {
      title: 'Set Filters & Grouping',
      description: 'Refine your data',
    },
    {
      title: 'Finalize & Preview',
  ...
  const scheduleReport = useCallback(async (values) => {
    ...
    if (editingSchedule) {
      ...
      setScheduledReports(prev => prev.map(s => s.id === editingSchedule.id ? updatedSchedule : s));
      notification.success({
        message: 'Schedule Updated',
        description: `Schedule "${values.name}" has been updated successfully.`,
        duration: 4,
      });
    } else {
      const newSchedule = {
      ...
      notification.success({ message: 'Report Scheduled', description: `Report "${values.name}" has been scheduled successfully.`, duration: 4 });
    }

    setEditingSchedule(null);
    scheduleForm.resetFields();
  }, [scheduleForm, editingSchedule, calculateNextRun]);
    ...
    <div style={{ padding: '24px' }}>
      ...
      <Tabs 
        ...
        items={[
          {
            key: 'builder',
            label: <span><BuildOutlined />Report Builder</span>,
            children: (
              <Card>
                <Empty
          ...
          ...(reports.custom ? [{
            key: 'custom',
            label: <span><TableOutlined />Custom Report</span>,
            children: (
              <Card
                title={
      ...
      <Modal
        ...
        open={reportBuilderVisible}
        onCancel={() => setReportBuilderVisible(false)}
        width={1000}
        footer={[
          <Button key="cancel" onClick={() => setReportBuilderVisible(false)}>
            Cancel
          </Button>,
          <Button key="back" onClick={() => setReportBuilderStep(reportBuilderStep - 1)} disabled={reportBuilderStep === 0}>
            Previous
          </Button>,
          <Button key="next" type="primary" onClick={() => setReportBuilderStep(reportBuilderStep + 1)} disabled={reportBuilderStep === reportBuilderSteps.length - 1}>
            Next
          </Button>,
          <Button key="submit" type="primary" onClick={generateCustomReport} loading={loading} disabled={reportBuilderStep !== reportBuilderSteps.length - 1}>
            Generate Report
          </Button>,
        ]}
      >
        <Steps current={reportBuilderStep}>
          {reportBuilderSteps.map(step => <Step key={step.title} title={step.title} description={step.description} />)}
        </Steps>

        <div style={{ marginTop: 24, padding: '24px', background: '#f5f5f5', borderRadius: '8px', minHeight: 450 }}>
          {reportBuilderStep === 0 && (
            <Form layout="vertical">
                ...
                <Col span={12}>
                  <Form.Item label="Report Name" required>
                    <Input 
                      ...
                      placeholder="e.g., Monthly Customer Revenue"
                    />
                  </Form.Item>
                </Col>
                <Col span={12}>
                  <Form.Item label="Report Type" required>
                    <Select 
                      ...
                      onChange={(value) => {
                        handleReportConfigChange('type', value);
              ...
              <Form.Item label="Description">
                <Input.TextArea 
                  ...
                  onChange={(e) => handleReportConfigChange('description', e.target.value)}
                  placeholder="A brief description of what this report shows."
                />
              </Form.Item>
              <Divider>Select Fields</Divider>
              <Transfer
                dataSource={availableFields[reportConfig.type] || []}
          ...
          {reportBuilderStep > 0 && <Empty description="This step will be implemented next." />}
        </div>
      </Modal>
      ...
      >
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    scheduleForm.setFieldsValue({
                      ...
                      time: schedule.time ? moment(schedule.time, 'HH:mm') : null,
                    });
                  }}
                >
                  Edit
        ...
        >
          <Title level={4}>{editingSchedule ? 'Edit Schedule' : 'Create Schedule'}</Title>
