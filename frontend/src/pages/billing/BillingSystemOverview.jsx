  ...
  const fetchSystemOverview = useCallback(async () => {
    ...
    try {
      ...
      setQuickStats({
        ...
        activeCustomers: parseInt(analyticsData.active_customers || healthResponse.data.active_customers || 0),
        overdueInvoices: parseInt(analyticsData.overdue_invoices || healthResponse.data.overdue_invoices || 0),
        recentPayments: parseFloat(analyticsData.recent_payments || healthResponse.data.recent_payments || 0),
        billingCycles: parseInt(healthResponse.data.recent_billing_runs || 0),
        successRate: parseFloat(analyticsData.success_rate || healthResponse.data.success_rate || 0),
        avgProcessingTime: parseFloat(healthResponse.data.avg_processing_time || analyticsData.avg_processing_time || 0)
      });
      
      // Use real chart data if available
      setRevenueChart(analyticsData.revenue_by_month || []);
      setPaymentMethodChart(analyticsData.payments_by_method || []);
      
      // Update performance metrics
      setPerformanceMetrics({
        databaseHealth: healthResponse.data.database_connection === 'ok' ? 100 : 0,
        avgProcessingTime: parseFloat(healthResponse.data.avg_processing_time || analyticsData.avg_processing_time || 0),
        invoiceGenerationRate: parseInt(healthResponse.data.invoice_rate || analyticsData.invoice_rate || 0),
        paymentProcessingRate: parseFloat(healthResponse.data.payment_success_rate || analyticsData.payment_success_rate || 0)
      });
              ...
              <Col span={6}>
                <Card size="small">
                  <Statistic
                    ...
                    valueStyle={{ color: '#1890ff' }}
                  />
                  <Progress percent={performanceMetrics.avgProcessingTime ? Math.min(100, Math.round(100 - performanceMetrics.avgProcessingTime * 10)) : 0} size="small" />
                </Card>
              </Col>
              <Col span={6}>
                <Card size="small">
                  <Statistic
                    ...
                    value={performanceMetrics.invoiceGenerationRate}
                    suffix="/hour"
                    valueStyle={{ color: '#722ed1' }}
                  />
                  <Progress percent={performanceMetrics.invoiceGenerationRate ? Math.min(100, Math.round(performanceMetrics.invoiceGenerationRate / 2)) : 0} size="small" />
                </Card>
              </Col>
              <Col span={6}>
                <Card size="small">
                  <Statistic
                    ...
                    value={performanceMetrics.paymentProcessingRate}
                    suffix="%"
                    precision={1}
                    valueStyle={{ color: '#13c2c2' }}
                  />
                  <Progress percent={performanceMetrics.paymentProcessingRate || 0} size="small" status="success" />
                </Card>
              </Col>
            </Row>
          </Card>
