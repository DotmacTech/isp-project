...
const IPv4NetworksPage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [networksRes, locationsRes, categoriesRes] = await Promise.all([
        apiClient.get('/v1/network/ipam/ipv4/'),
  ...
  const handleAdd = () => {
    ...
    form.resetFields();
    form.setFieldsValue({ network_type: 'endnet', type_of_usage: 'management' });
  ...
  const handleDelete = async (id) => {
    try {
      await apiClient.delete(`/v1/network/ipam/ipv4/${id}`);
      message.success('IPv4 Network deleted successfully');
      fetchData();
    } catch (error) {
      message.error(error.response?.data?.detail || 'Failed to delete IPv4 Network.');
    }
  };
  ...
  const handleFormFinish = async (values) => {
    const url = editingNetwork
      ? `/v1/network/ipam/ipv4/${editingNetwork.id}`
      : '/v1/network/ipam/ipv4/';
    const method = editingNetwork ? 'put' : 'post';

    try {
      await apiClient({ method, url, data: values });
      message.success(`IPv4 Network ${editingNetwork ? 'updated' : 'created'} successfully.`);
      setIsModalVisible(false);
      fetchData();
    } catch (error) {
      message.error(error.response?.data?.detail || 'Failed to save IPv4 Network.');
    }
  };

  const columns = [
    { title: 'Network', dataIndex: 'network', key: 'network', render: (text, record) => `${record.network}/${record.mask}` },
  ...
  return (
    <Card>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <Title level={2} style={{ margin: 0 }}><NodeIndexOutlined /> IPv4 Networks</Title>
        <Button type="primary" icon={<PlusOutlined />} onClick={handleAdd}>
          Add IPv4 Network
        </Button>
      </div>
      <Table dataSource={networks} columns={columns} rowKey="id" loading={loading} />
      <Modal title={editingNetwork ? 'Edit IPv4 Network' : 'Add IPv4 Network'} open={isModalVisible} onCancel={handleModalCancel} footer={null} destroyOnHidden>
        <Form form={form} layout="vertical" onFinish={handleFormFinish}>
          <Form.Item name="network" label="Network Address" rules={[{ required: true }]}><Input placeholder="e.g., 192.168.1.0" /></Form.Item>
          <Form.Item name="mask" label="Mask" rules={[{ required: true }]}><InputNumber min={0} max={32} style={{ width: '100%' }} placeholder="e.g., 24" /></Form.Item>
          <Form.Item name="title" label="Title" rules={[{ required: true }]}><Input placeholder="e.g., Customer Pool A" /></Form.Item>
          ...
          <Form.Item name="type_of_usage" label="Type of Usage" rules={[{ required: true }]}><Select><Option value="management">Management</Option><Option value="pool">Pool</Option><Option value="static">Static</Option></Select></Form.Item>
...
export default IPv4NetworksPage;