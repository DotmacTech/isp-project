...
const UserManagementPage = () => {
  ...
  const fetchData = useCallback(async (params = {}) => {
      try {
        ...
        const pageUsers = usersResponse.data.items;
        setUsers(pageUsers);
        ...
        if (pageUsers.length > 0) {
          ...
          for (const { userId, roles } of rolesResults) {
            newUserRoles[userId] = roles.map(ur => ur.role.name);
            const permissions = roles.flatMap(ur => ur.role.permissions.map(p => p.code));
  ...
  const columns = [
    ...
    { title: 'Kind', dataIndex: 'kind', key: 'kind', render: kind => <Tag>{kind.toUpperCase()}</Tag> },
    { 
      title: 'Status', 
      dataIndex: 'is_active', 
      key: 'is_active', 
    ...
    },
    {
      ...
      dataIndex: 'id',
      key: 'roles',
      render: (userId) => (userRoles[userId] || []).join(', ') || <Spin size="small" />,
    },
    {
      title: 'Permissions',
      dataIndex: 'id',
      ...
      render: (userId) => {
        ...
        return perms.join(', ') || <Spin size="small" />;
      },
    },
    {
      title: 'Actions',
  ...
  return (
    <div>
      ...
      />
      <Modal title={editingUser ? 'Edit User' : 'Create User'} open={isUserModalVisible} onCancel={handleUserModalCancel} footer={null} destroyOnClose>
      ...
      <Modal title={`Edit Roles for ${editingRolesUser?.full_name}`} open={isRolesModalVisible} onCancel={handleRolesModalCancel} footer={null} destroyOnClose>
