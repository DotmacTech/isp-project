...
const LookupTable = ({ title, endpoint }) => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const response = await apiClient.get(`/v1/network/monitoring/config/${endpoint}/`);
 ...
 const handleFormFinish = async (values) => {
  ...
  return (
    <div>
      ...
      <Modal title={`${editingItem ? 'Edit' : 'Add'} ${title.slice(0, -1)}`} open={isModalVisible} onCancel={() => setIsModalVisible(false)} footer={null} destroyOnClose>
