...
def require_super_admin(current_user: models.User = Depends(security.get_current_user), db: Session = Depends(get_db)):
    user_roles = crud.get_user_roles(db, current_user.id)
    if not any((getattr(r, "role", None) and getattr(r.role, "name", None) == "Super Admin") or (hasattr(r, "role_id") and getattr(r, "role_id", None) and crud.get_role(db, r.role_id).name == "Super Admin") for r in user_roles):
...
def read_users(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    # Only users with "system.manage_users" can access this endpoint
...
@role_router.post("/", response_model=schemas.RoleResponse, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_super_admin)])
...
@role_router.get("/{role_id}", response_model=schemas.RoleResponse, dependencies=[Depends(require_super_admin)])
...
@role_router.get("/", response_model=list[schemas.RoleResponse], dependencies=[Depends(require_super_admin)])
def read_roles(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    # Only users with "system.manage_roles" can access this endpoint
    roles = crud.get_roles(db, skip=skip, limit=limit)
    return roles

@role_router.put("/{role_id}", response_model=schemas.RoleResponse, dependencies=[Depends(require_super_admin)])
...
@role_router.delete("/{role_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_super_admin)])
...
@permission_router.post("/", response_model=schemas.PermissionResponse, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_super_admin)])
...
@permission_router.get("/{permission_id}", response_model=schemas.PermissionResponse, dependencies=[Depends(require_super_admin)])
...
@permission_router.get("/", response_model=list[schemas.PermissionResponse], dependencies=[Depends(require_super_admin)])
def read_permissions(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), current_user: models.User = Depends(security.get_current_user)):
    # Only users with "system.manage_roles" can access this endpoint
    print("DEBUG: Current user:", current_user.email if current_user else None)
    user_permissions = crud.get_user_permissions(db, current_user.id) if current_user else None
...
@user_role_router.post("/", response_model=schemas.UserRoleResponse, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_super_admin)])
...
@user_role_router.delete("/", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_super_admin)])
...
@user_role_router.get("/{user_id}", response_model=list[schemas.UserRoleResponse], dependencies=[Depends(require_super_admin)])
